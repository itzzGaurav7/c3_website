...
import { Menu, X, Hexagon, CalendarDays, Layers, Users, CreditCard, ArrowUpRight } from 'lucide-react';
...
const Navbar = () => {
  ...
  return (
    <>
        ...
        <div className="pointer-events-auto glass-panel rounded-full flex items-center p-1.5 gap-2 ring-1 ring-black/5">
          ...
          <div className="pl-5 pr-6 flex items-center gap-3 border-r border-slate-200/50">
            ...
            <div className="flex flex-col leading-none">
              <span className="font-tech font-bold text-ind-800 tracking-tight text-lg">C³–26</span>
            </div>
          </div>
            ...
            {navLinks.map((link) => (
              ...
              >
                ...
                <span className="relative z-10 flex items-center gap-2">
                   <link.icon size={16} className={`transition-all duration-300 ${activeTab === link.id ? 'text-ind-600 scale-110' : 'opacity-70 group-hover:opacity-100'}`} />
                   {link.name}
                </span>
              </a>
            ))}
      ...
      <div className="lg:hidden fixed top-0 w-full z-50 glass-panel border-b-0 rounded-none px-4 py-3 flex justify-between items-center">
         <div className="flex items-center gap-2 font-tech font-bold text-lg text-slate-800">
             <Hexagon className="text-ind-600 fill-ind-100" /> C³–26
         </div>
         <button onClick={() => setIsMobileOpen(!isMobileOpen)} className="p-2 bg-slate-100 rounded-full text-slate-600">
       ...
       <div className="lg:hidden fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 py-3 flex justify-between items-center">
         ...
         <button onClick={() => setIsMobileOpen(!isMobileOpen)} className="p-2 bg-slate-100 rounded-lg">
            {isMobileOpen ? <X size={20} /> : <Menu size={20} />}
         </button>
      </div>
        ...
        {isMobileOpen && (
            ...
            >
                <div className="p-4 space-y-1">
                    ...
                    <a href="https://forms.gle/w6DpCp5WcgJ7jbaC8" className="block w-full text-center bg-ind-900 text-white p-3 rounded-lg font-bold font-tech">
                        REGISTER NOW
                    </a>
                </div>
            </motion.div>
