...
const PersonCard = ({ name, role, desc, phone, isLead }) => (
  <div
    className={`p-4 transition-all rounded-lg flex gap-4 items-center ${
      isLead
  ...
  >
    <div className="flex-1">
      ...
      {desc && (
        <p className={`text-xs leading-relaxed ${isLead ? 'text-ind-200' : 'text-slate-600'}`}>
          {desc}
        </p>
      )}
      ...
      {phone && (
        <div className="mt-3 flex items-center gap-2">
          <PhoneCall size={12} className={isLead ? 'text-ind-300' : 'text-ind-500'} />
          <a 
            href={`tel:${phone}`} 
            className={`text-xs ${isLead ? 'text-ind-200 hover:text-white' : 'text-slate-600 hover:text-ind-600'}`}
          >
            {phone}
          </a>
        </div>
    ...
    <div className={`w-20 h-20 rounded-lg overflow-hidden flex-shrink-0 ${isLead ? 'ring-2 ring-ind-400' : ''}`}>
      <img
...
const Committee = () => {
  return (
    <section id="committee" className="py-24 bg-gradient-to-b from-slate-50 to-white">
