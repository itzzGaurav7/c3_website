...
const events = [
  ...
  { 
    ...
    desc: 'Last date for standard registration.',
    icon: Calendar,
    status: 'upcoming'
  },
  { 
    ...
    <div className={`relative flex items-center justify-between md:justify-normal mb-12 last:mb-0 ${isEven ? 'md:flex-row-reverse' : ''}`}>
      ...
      <div className="absolute left-6 md:left-1/2 -translate-x-1/2 z-10 flex flex-col items-center">
        <div className={`w-8 h-8 rounded-full border-4 flex items-center justify-center bg-white transition-all duration-500
          ${evt.status === 'active' ? 'border-saf-500 shadow-[0_0_20px_rgba(99,102,241,0.4)] scale-110' : 'border-slate-200'}
        `}>
          <div className={`w-2.5 h-2.5 rounded-full ${evt.status === 'active' ? 'bg-saf-600 animate-pulse' : 'bg-slate-300'}`}></div>
        </div>
      </div>
      ...
      <motion.div 
        ...
        `}
      >
        <div className="glass-card p-6 rounded-2xl group relative hover:border-saf-300/50">
          ...
          <div className="flex items-start justify-between mb-2">
            ...
            <evt.icon size={20} className="text-slate-300 group-hover:text-saf-500 transition-colors duration-300" />
          </div>

          <h3 className="font-tech text-xl font-bold text-slate-900 mb-1 group-hover:text-saf-700 transition-colors">
            {evt.title}
          </h3>
  ...
  return (
    <section id="timeline" className="py-32 relative bg-slate-50 overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full opacity-30 pointer-events-none">
         <div className="absolute right-0 top-1/4 w-96 h-96 bg-saf-100/50 rounded-full blur-3xl"></div>
